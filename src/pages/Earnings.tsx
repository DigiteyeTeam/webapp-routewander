  ...
  return (
    <MotionPage className="p-6 md:p-12 lg:p-16 w-full max-w-7xl mx-auto space-y-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-primary text-on-primary rounded-3xl p-8 md:p-12 shadow-[0_8px_30px_rgba(220,38,38,0.3)] relative overflow-hidden flex flex-col justify-between min-h-[320px]">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 mt-8">
            <div className="flex items-center gap-3 bg-white/20 px-4 py-2 rounded-lg backdrop-blur-sm">
              ...
              <span className="font-bold">+12.5% เทียบเดือนที่แล้ว</span>
            </div>
            <button className="w-full sm:w-auto h-14 px-8 bg-white text-primary font-bold rounded-full shadow-lg hover:scale-105 active:scale-95 transition-transform text-lg whitespace-nowrap">
              ถอนเงิน
            </button>
          </div>
        </div>
            ...
            <tbody className="divide-y divide-surface-variant">
              {transactions.map((tx, i) => (
                <tr key={i} className="hover:bg-surface-container-low/50 transition-colors">
                  ...
                  <td className={`p-4 md:p-6 font-bold text-right ${tx.isSale ? 'text-primary' : 'text-on-surface'}`}>{tx.amount}</td>
